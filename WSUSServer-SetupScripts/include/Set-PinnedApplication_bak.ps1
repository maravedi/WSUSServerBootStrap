...
# DATE  : 06.08.2010  
#  
# COMMENT: Module with the ability to pin and unpin programs from the taskbar and the Start-menu in Windows 7
...
# 1.3 05.18.2018 - Update by David Frazer to allow for application paths
#                   that include switches.
#  
###########################################################################" 
<#  
.SYNOPSIS  
...
.EXAMPLE
Set-PinnedApplication -Action PinToTaskBar -FilePath "C:\WINDOWS\system32\perfmon.exe" -FileSwitch "/res"
.EXAMPLE
Set-PinnedApplication -Action UnpinFromTaskbar -FilePath "C:\WINDOWS\system32\perfmon.exe" -FileSwitch "/res"
#>  
[CmdletBinding()] 
param( 
...
function InvokeVerb { 
  Param([string]$FilePath,$verb,$FileSw) 
  $verb = $verb.Replace("&","") 
...
If($FileSwitch) {
  InvokeVerb -FilePath $FilePath -Verb $(GetVerb -VerbId $verbs.$action) -FileSw $FileSwitch
} Else {
